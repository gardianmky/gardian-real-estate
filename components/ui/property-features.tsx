...
export default function PropertyFeatures({
  ...
  const itemClasses = {
    horizontal: "flex items-center text-sm text-gray-700 font-medium",
...
export function PropertyFeaturesInline({
  listing,
  className = "",
}: {
  listing: Listing;
...
export function PropertyFeaturesCompact({
  listing,
  className = "",
}: {
  listing: Listing;
...
export function PropertyFeaturesCard({
  ...
  const features = formatPropertyFeatures(listing, "standard");
    ...
    <div className={`flex gap-4 ${className}`}>
      {features.map((feature) => (
        ...
        >
          ...
          <span className="text-xs text-gray-600 ml-1">{feature.label}</span>
        </div>
      ))}
    </div>
  );
